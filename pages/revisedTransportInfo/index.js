...
Page({
  ...
  switchToAddCarInfo() {
    wx.navigateTo({
      url: '../addCarInfo/index?isFromRevised=isFromRevised',
    })
  },

  switchToFaCheInfo() {
    wx.navigateTo({
      url: '../faCheInfo/index?isFromTransport=isFromTransport&isFromRevised=isFromRevised',
    })
  },
  switchToShouCheInfo() {
    wx.navigateTo({
      url: '../shouCheInfo/index?isFromTransport=isFromTransport&isFromRevised=isFromRevised',
    })
  },
  bindInvoiceChange(e) {
    this.setData({
    ...
    if (e.detail.value == 0) {
      wx.navigateTo({
        url: '../normalInvoice/index?isFromRevised=isFromRevised',
      })
    } else if (e.detail.value == 1) {
      wx.navigateTo({
        url: '../vatInvoice/index?isFromRevised=isFromRevised',
      })
    }
  },
  bindCall() {
  ...
  switchToMangeCarsInfo() {
    wx.navigateTo({
      url: '../mangeCarsInfo/index?isFromRevised=isFromRevised',
    })
  },
  ...
  bindSaveOrder() {
    if (this.data.carsArray.length <= 0) {
      ...
      storageData.splice(this.data.indexFromRevisePage,1,orderInfo)
      console.log(storageData)
...
// 确认下单
  bindConfirmOrder(){
    if (this.data.carsArray.length <= 0) {
      ...
      savedOrders.splice(this.data.indexFromRevisePage,1)
