  ...
  switchToAddCarInfo() {
    ...
    if (this.data.isFromTransport||this.data.isFromConfirm) {
      if (!this.data.nameValue) {
        ...
        wx.removeStorageSync("faCheInfoArray");
        let regionString = "";
        this.data.region.forEach((item) => {
          regionString += item + ''
        })
        ...
        wx.setStorageSync('faCheInfoArray', faCheInfo)
...
onLoad: function(options) {
  ...
  if(options.isFromTransport){
    ...
    if(faCheInfo){
      this.setData({
        firstName: faCheInfo.firstName,
        nameValue: faCheInfo.nameValue ,
        phoneValue: faCheInfo.phoneValue,
        styleArray: ["自送", "上门提车", "拖车上门"],
        styleIndex: this.data.styleArray.indexOf(faCheInfo.style),
        region: [faCheInfo.region,"",""],
        detailAddress: faCheInfo.detailAddress,
        phoneValid: true
